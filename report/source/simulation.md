...
- $f(x,y,z)<0$ point inside the solid volume;
- $f(x,y,z)<0$, point is on the surface of the volume;
- $f(x,y,z)>0$, point is outside the solid volume.
...
where:
- $g(x,y,z)$ is the random component;
- $m(x,y,z)$ is a dome function;
- $u$ is a threshold.
